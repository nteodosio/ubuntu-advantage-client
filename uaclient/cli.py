...
def attach_parser(parser):
    ...
    parser.description = (
        ...
        + "\n\n"
        + (
            "When running this command without a token, we will use the "
            "magic-attach\nflow to attach this machine into a Ubuntu Pro "
            "subsctription without a token\nLearn more at: magic-attach-url"
        )
    )
    parser._optionals.title = "Flags"
    parser.add_argument(
...
def _magic_attach(args, *, cfg, **kwargs):
    event.info("Initiating magic-attach flow...")
    initiate_resp = _initiate(cfg=cfg)

    event.info("Please sign in to advantage portal:")
    event.info("https://ubuntu.com/pro")
    event.info(
        "And provide the following user code: {}".format(
    ...
    event.info("Attaching the machine")
